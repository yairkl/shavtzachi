...
class ShavtzachiDB:
    ...
    def get_soldier_by_id(self, soldier_id: int, include_skills=True, include_unavailabilities=False, include_excluded_posts=True) -> Optional[Soldier]:
        query = self.session.query(Soldier).filter(Soldier.id == soldier_id)
        if include_skills:
            query = query.options(joinedload(Soldier.skills))
        if include_unavailabilities:
            query = query.options(joinedload(Soldier.unavailabilities))
    ...
    def batch_upsert_soldiers(self, soldiers_data: List[dict]):
        for s in soldiers_data:
            existing = self.get_soldier_by_name(s["name"])
            if existing:
                self.update_soldier(existing.id, s["name"], s["skills"], s.get("division"), s.get("excluded_posts", []))
            else:
                self.create_soldier(s["name"], s["skills"], s.get("division"), s.get("excluded_posts", []))
        self.session.commit()
    ...
    def batch_upsert_posts(self, posts_data: List[dict]):
        for p in posts_data:
            existing = self.get_post_by_name(p["name"])
            if existing:
                self.update_post(
                    p["name"], p["shift_length_hours"], p["start_time"], p["end_time"],
                    p["cooldown_hours"], p["intensity_weight"], p["slots"],
                    p.get("is_active", True), p.get("active_from"), p.get("active_until")
                )
            else:
                self.create_post(
                    p["name"], p["shift_length_hours"], p["start_time"], p["end_time"],
    ...
    def prefetch_assignments(self, start: datetime, end: datetime):
        # SQLAlchemy handles its own caching/optimization, so this is a no-op for parity.
        pass
    ...
    def count_assignments(self) -> int:
        return self.session.query(Assignment).count()
    ...
    def delete_assignments_for_soldier(self, soldier_id: int):
        self.session.query(Assignment).filter(Assignment.soldier_id == soldier_id).delete(synchronize_session=False)

    def clear_all_data(self):
        from models import Soldier, Post, Skill, Shift, Assignment, PostTemplateSlot, soldier_skill_table, Unavailability, Division
        self.session.execute(soldier_skill_table.delete())
        self.session.query(Assignment).delete()
        self.session.query(Shift).delete()
        self.session.query(Unavailability).delete()
        self.session.query(PostTemplateSlot).delete()
        self.session.query(Post).delete()
        self.session.query(Soldier).delete()
        self.session.query(Skill).delete()
        self.session.query(Division).delete()
        self.session.commit()
    ...
    def clear_assignments_in_range(self, start: datetime, end: datetime):
        shift_ids_query = self.session.query(Shift.id).filter(
            Shift.start >= start,
            Shift.start < end
        )
        self.session.query(Assignment).filter(Assignment.shift_id.in_(shift_ids_query)).delete(synchronize_session=False)

    def save_assignments_to_grid(self, assignments_data: list, start_date: datetime, end_date: datetime):
        self.clear_assignments_in_range(start_date, end_date)
        for a in assignments_data:
            post = self.get_post_by_name(a['post_name'])
            if not post: continue
            shift = self.get_or_create_shift(post, a['start'], a['end'])
            soldier = self.get_soldier_by_name(a['soldier_name'])
            if soldier:
                self.add_assignment(soldier.id, shift.id, a['role_id'])
        self.commit()
    ...
    def get_all_unavailabilities(self) -> List[Unavailability]:
        return self.session.query(Unavailability).all()
...
def database_sqlite_engine_override(eng):
    global engine, Session
    engine = eng
    Session = sessionmaker(bind=engine)
...
if __name__ == '__main__':
    ...
    init_db(create_engine('sqlite:///data.db'))
