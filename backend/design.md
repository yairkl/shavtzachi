...
| **`Config`** | `key`, `value` | Global settings (GSheets only: spreadsheet IDs). |

---

## 5. Multi-Backend Support

The system supports two interchangeable database backends, abstracting the `ShavtzachiDB` interface:

### 5.1. SQLite Backend
- **Use Case:** Local, offline-first scheduling.
- **Storage:** A local `data.db` file next to the application binary.
- **Implementation:** SQLAlchemy with `Assignment`, `Soldier`, and `Post` models.

### 5.2. Google Sheets Backend
- **Use Case:** Collaborative scheduling and cloud-synced data.
- **Storage:** Data is mapped to specific tabs in a Google Spreadsheet.
- **Authentication:** OAuth 2.0 with a dedicated "Login" flow in the application.
- **Caching:** TTL-based local cache to optimize API quota usage and performance.
...
### 6.2. Post & Template Configuration
...
### 6.3. The Scheduler Interface (Gantt View)
...
* **Publish & Commit:** Once the commander finalizes the schedule, clicking "Publish" writes the records to the database (SQLite or GSheets) and incrementally updates the `history_score`.

### 6.4. Desktop Application Lifecycle
To provide a native experience without a complex GUI wrapper:
* **Entry Point:** The application starts a local FastAPI server and opens the default system browser.
* **Heartbeat Mechanism:** The frontend pings the backend every 5 seconds.
* **Automatic Shutdown:** If no heartbeats are received for 15 seconds (indicating the browser tab was closed), the backend process automatically terminates.